...
if ( ! function_exists( 'inspiry_recaptcha_callback_generator' ) ) {
	...
	function inspiry_recaptcha_callback_generator() {
		...
		$lang       = get_locale();
		$public_key = $theme_options['recaptcha_public_key'];
        ...
        <script type="text/javascript">
            ...
            var inspirySiteKey = '<?php echo $public_key; ?>';
	...
	function inspiry_verify_google_recaptcha() {
		...
		if ( $resp->isSuccess() ) {
			...
			$error_messages      = array(
				'missing-input-secret'   => __( 'The secret parameter is missing.', 'framework' ) ,
				'invalid-input-secret'   => __( 'The secret parameter is invalid or malformed.', 'framework' ),
				'missing-input-response' => __( 'The response parameter is missing.', 'framework' ),
				'invalid-input-response' => __( 'The response parameter is invalid or malformed.', 'framework' ),
			);
