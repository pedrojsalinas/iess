...
if ( ! function_exists( 'inspiry_gdpr_checkbox' ) ) {

	function inspiry_gdpr_checkbox( $form = 'contact' ) {

		global $allowed_html_tags;

		$gdpr_checkbox_label     = inspiry_get_option( 'gdpr_label_text' );
		$gdpr_text               = inspiry_get_option( 'gdpr_text' );
		$gdpr_validation_message = inspiry_get_option( 'gdpr_validation_message' );

		?>
		<div class="gdpr-checkbox-wrapper">
			<label for="<?php echo $form ?>-form-gdpr-checkbox">
				...
				<?php } ?>
				<input id="<?php echo $form ?>-form-gdpr-checkbox"
					   class="required"
					   name="<?php echo $form ?>-form-gdpr-checkbox"
					   type="checkbox"
...
if ( ! function_exists( 'inspiry_process_slugs' ) ) :

	function inspiry_process_slugs( $existing_slug, $slug_in_theme_options ) {

		$get_the_slug = inspiry_get_option( $slug_in_theme_options );

		return ( ( ! empty( $get_the_slug ) ) ? $get_the_slug : $existing_slug );

	}

	add_filter( 'inspiry_filter_slugs', 'inspiry_process_slugs', 10, 3 );

endif;

if ( ! function_exists( 'inspiry_filter_news' ) ) :

	function inspiry_filter_news( $args ) {

		global $theme_options;

		if ( ( $theme_options[ 'home_news_type' ] == 'category' ) && ( ! empty( $theme_options[ 'home_news_category' ] ) ) ) {
			$home_news_categories = $theme_options[ 'home_news_category' ];
			if ( is_array( $home_news_categories ) ) {
				$args[ 'category__in' ] = $home_news_categories;
			} else {
				$args[ 'cat' ] = intval( $home_news_categories );
			}

		} elseif ( ( $theme_options[ 'home_news_type' ] == 'tag' ) && ! empty( $theme_options[ 'home_news_tag' ] ) ) {
			$home_news_tags = $theme_options[ 'home_news_tag' ];
			if ( is_array( $home_news_tags ) ) {
				$args[ 'tag__in' ] = $home_news_tags;
			} else {
				$args[ 'tag_id' ] = intval( $home_news_tags );
			}
		} elseif ( ( $theme_options[ 'home_news_type' ] == 'author' ) && ! empty( $theme_options[ 'home_news_author' ] ) ) {
			$home_news_authors = $theme_options[ 'home_news_author' ];
			if ( is_array( $home_news_authors ) ) {
				$args[ 'author__in' ] = $home_news_authors;
			} else {
				$args[ 'author' ] = intval( $home_news_authors );
			}
		}
	...
	add_filter( 'inspiry_filter_news', 'inspiry_filter_news');
